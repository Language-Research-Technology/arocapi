...
const file = async (fastify, opts) => {
    const { fileHandler } = opts;
    fastify.withTypeProvider().head('/file/:id', {
        schema: {
            params: paramsSchema,
        },
    }, async (request, reply) => {
        ...
        try {
            ...
            const metadata = await fileHandler.head(file, {
                request,
                fastify,
            });
            if (!metadata) {
    ...
    fastify.withTypeProvider().get('/file/:id', {
        schema: {
            params: paramsSchema,
        ...
        try {
            ...
            console.error(`Unexpected file result type: ${result.type}`);
