...
describe('File Route', () => {
    ...
    describe('GET /file/:id', () => {
        it('should stream file content successfully', async () => {
            ...
            expect(mockFileHandler.get).toHaveBeenCalledWith(mockFile, expect.objectContaining({
    ...
    describe('HEAD /file/:id', () => {
        it('should return headers without body using head handler', async () => {
            ...
            expect(response.headers.etag).toBe('"abc123"');
            expect(response.body).toBe('');
            expect(mockFileHandler.head).toHaveBeenCalledWith(mockFile, expect.objectContaining({
                request: expect.any(Object),
                fastify: expect.any(Object),
            }));
        });
    ...
    describe('GET /file/:id - exhaustiveness check', () => {
        it('should handle unexpected file result type', async () => {
            ...
            vi.mocked(mockFileHandler.get).mockResolvedValue(invalidResult);
            const response = await fastify.inject({
                method: 'GET',
                url: `/file/${encodeURIComponent('http://example.com/file/test.wav')}`,
            });
